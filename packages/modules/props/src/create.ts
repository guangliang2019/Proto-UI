...
export function createPropsModule<P extends PropsBaseType>(ctx: ModuleFactoryArgs): PropsModule<P> {
    ...
    build: ({ init, caps }) => {
      ...
      return {
        facade: {
          ...
          watchRaw: (keys, cb) => impl.watchRawKeys(keys as any, cb as any, true),
