...
function groupImports(componentIds) {
  const packages = new Map();

  for (const componentId of componentIds) {
    const entry = getComponentEntry(componentId);
    if (!entry) continue;
    const set = packages.get(entry.packageName) ?? new Set();
...
export function renderHostIndex(host, componentIds) {
...
export function renderRootIndex(componentsByHost) {
  const lines = createFileHeader();
  let wrote = false;
...
export async function writeGeneratedIndexes(paths, config) {
