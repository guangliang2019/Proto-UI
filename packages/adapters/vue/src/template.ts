// packages/adapters/vue/src/template.ts
import {
  mergeTwTokensV0,
...
function renderChild(
  ...
  if (isReservedType(t) && t.kind === 'slot') {
    if ((child as any).children != null) {
      throw new Error('[Vue Adapter] slot node must not have children in v0.');
    }
    if ((child as any).style != null) {
      throw new Error('[Vue Adapter] slot node must not have style in v0.');
    }
    if ((t as any).name) {
      throw new Error('[Vue Adapter] named slot is not supported in v0.');
    }
    if (ctx.slotUsed) {
      throw new Error('[Vue Adapter] multiple slot is not supported in v0.');
    }
    ctx.slotUsed = true;
...
export function renderTemplateToVue(
  runtime: VueRuntime,
