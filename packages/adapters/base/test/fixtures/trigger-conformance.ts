...
export function triggerAdapterConformance(
  ...
  describe(`${name}: Trigger group translation`, () => {
    it('T-AS-TRIGGER-0002-CASE-ADAPTER: shares a three-member route and preserves host-local events across fresh owners', async () => {
      ...
      const local = [0, 0, 0];
      const protos = semantic.map((_, index) => definePrototype({
        name: `trigger-${name}-catalog-${index}`,
        setup(def) {
          asTrigger();
          def.event.on('press.commit', () => { semantic[index]++; });
          def.event.on('host:catalog-local', () => { local[index]++; });
          return (r) => r.slot();
        },
      }));
      for (let generation = 0; generation < 2; generation++) {
        const mounted = await mount([{ proto: protos[0], children: [
          { proto: protos[1], children: [{ proto: protos[2] }] },
        ] }]);
