...
export function renderLucideChessKnightIcon(
  renderer: SvgRendererHandle,
  options: RenderLucideShapeOptions = {}
) {
  return renderLucideShape(renderer, LUCIDE_CHESS_KNIGHT_SHAPE_FACTORY, options);
}

const fixed = createLucideFixedIcon({
  asHookName: 'as-lucide-chess-knight-icon',
  prototypeName: 'lucide-chess-knight-icon',
  shapeFactory: LUCIDE_CHESS_KNIGHT_SHAPE_FACTORY,
});

export const asLucideChessKnightIcon = fixed.asHook;
export const lucideChessKnightIcon = fixed.prototype;
export default lucideChessKnightIcon;
