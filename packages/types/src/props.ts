...
// 对于一个值类型 V，允许哪些 type？（v0 简化）
// - boolean -> type: "boolean" | "any"
...
// 对于某个具体 type K，在值类型 V 下允许的 empty 选项：
// - empty:"accept" 只有当 V 含 null 时才允许
type AllowedEmptyFor<V> =
  HasNull<V> extends true
    ? EmptyBehavior // accept/fallback/error 都允许
    : Exclude<EmptyBehavior, 'accept'>;

// 构造一个 Spec shape：固定 type，并把 empty 限制住
type SpecShape<K extends PropType, V> = Omit<PropSpec, 'type' | 'empty'> & {
  type: K;
