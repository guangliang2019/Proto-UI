---
title: '第五章：翻译层'
description: '说明 Prototype 如何经由翻译层进入具体 Host，并明确能力边界、翻译结果与证据范围。'
---
...
以 Switch family 为例，相关 Prototypes 可以说明 Root 接受哪些 Props、保存怎样的 State、响应哪些 Event、向 User 展示什么 Feedback、向 App Maker 暴露什么信息，以及 Root 与 Thumb 如何组成同一个 Anatomy family。它们分别描述了各个交互主体必须履行的义务。

但是 React、Flutter 和 Qt 并不直接认识这些义务。

以 React Web 为例，它面对的是 component owner、props、commit 与 DOM；Flutter 有 widget、element 和 render object；Qt 则有 widget、property、signal 与自己的事件系统。三者对于结构、状态、输入和生命周期都有各自的表达方式。

Proto UI 把这些具体的技术环境称为 Host，中文译名为“宿主”。把 Prototype 所声明的义务映射到一个 Host 的工作，则由翻译层负责。
...
这里的 Host artifact，指翻译之后实际存在于目标技术中的产物。它并不一定是一份生成出来的代码文件：它可以是运行中的 React component、DOM 与事件绑定，可以是 Custom Element class 及其实例，也可以是平台 widget、controller、样式、注册信息，或者由这些内容共同组成的运行结果。
...
然而，如果每增加一种 Host，都要以接近开发 React Native 或 Flutter 的成本重新建设一整套跨端框架，那么 Prototype 的落地性价比就会大打折扣。它可能依然是一套有用的组件分析理论，却很难成为可以持续扩展的工程基础。

因此，翻译层还必须回答另一个问题：怎样把翻译器的开发，从一项边界模糊的巨大工程，拆成可以复用、实现和验证的工作？
...
前四章介绍过 Props、Event、Feedback、Context、State 等语义。不同 Host 最终要让这些语义运行起来，其中大量的逻辑并不需要每个翻译器重新发明。

Proto UI 用 Module 封装可复用的语义实现与责任。例如，Props Module 负责 Props 的声明、默认值、fallback 逻辑与变化处理；State Module 负责 Component instance 内部状态的持有与访问；Event 相关 Module 则负责把输入事件组织成 Prototype 可以处理的形式。

Module 不是前几章概念目录的一一翻版。一个语义领域可能需要多个 Module 协作（如 event 下所属的 Module 就很多）；有些语义领域在彼此协作时可能产生完全独立的语义，需要单独的 Module 支持（如 expose-state）；有些跨领域的执行秩序需要由 Runtime（Proto UI 里负责编排 Module、生命周期与渲染，并衔接 Adapter/Host 的架构分层）与翻译器共同承担，而不是独立的 Module（如 lifecycle）；也有些 Module 只处理 Proto UI 内部的逻辑，完全不需要向 Host 索取能力（如 state）。

只有当 Module 确实需要接触具体 Host 时，它才会索取 Host Capability。
...
1. 这个翻译器准备支持哪些受治理的语义能力？相应 profile 对哪些 Module 已有经过审查的支持或省略结论，Runtime 与 Adapter 又将如何接入它们？
...
从 Prototype 到 Host artifact，表现形式几乎必然发生变化，但交互语义不必然因此受损。

例如，Switch 向 App Maker 发出的 `checkedChange` Expose event，在 React Web 的宿主 API 中可以由 `onCheckedChange` callback prop 承接，在 Web Component 中可以表现为 `CustomEvent`。两边的 API 和 Host artifact 都不同，但 Prototype 中的信息通路身份并未改变；只要信息方向、触发条件与 payload 义务仍然成立，这次翻译就没有因为“长得不一样”而丢失语义。

不过，Host 之间确实存在不能轻易抹平的差异。翻译结果需要诚实地区分三种情况。

### Faithful

faithful 表示在已经声明的 Host、profile 与运行条件下，Prototype 的必需义务都得到保持。
...
其中最重要的是 `authorized` 和 `bounded`：翻译器不能因为实现困难，就自行放宽 Prototype 的要求；被放弃的内容也不能含糊地藏在“兼容性有限”之类的描述里。

### Unsupported

如果 Host 无法保持某项必需的义务，又不存在经过授权的替代方案，那么结果就是 unsupported。

这对于 Host 或翻译器来说完全不是一种批评。实际上，明确承认无法支持，往往比生成一个看起来可以运行、实际上已经改变 Component 身份的产物更可靠。
...
既然要挑战边界，我们来举一个在 GUI 世界里不那么常规的例子。

假设我们要把一个官方 Prototype 翻译到 Terminal UI。这个翻译器的首要目标当然不是复制 GUI 的每一个像素，而是应该产出符合 TUI 操作与呈现习惯、并且真正可用的 Component。
...
这张表不需要记忆。它的作用只是提醒我们：讨论翻译器时，这四个问题不能互相回答。
...
当然，“尽早”不等于猜测。有些结论在编译时就能确定，有些要等 Adapter 看见具体 Prototype 才能判断，还有些只有在 Host session 建立、真实能力完成协商之后才会成立。
...
接下来的问题是：当两个 Host artifact 使用不同的结构、事件系统与渲染方式时，我们凭什么说它们仍然实现了同一个 Component？一致性应该严格到行为、结构，还是像素？如果没有一个标准，翻译工作很难说有尽头，也没有明确的目标。

下一章，我们将讨论 Proto UI 如何理解和判断一致性，以及这种一致性成立的条件与边界。
