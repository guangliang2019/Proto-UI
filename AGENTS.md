...
Read `internal/agent-operations/contributor-agents.md` before selecting work autonomously. Local comprehension assessment is advisory in `human-assisted` mode and a binding task/review ceiling in `autonomous` mode. It never grants GitHub permission, proves identity, or predicts acceptance. Current authorization, repository rules, live platform permission, relevant Discord or Poppy trust, provenance, CI, independent review, and exact-target mutation controls remain execution conditions that the workflow revalidates at the action boundary.
...
- Commit, push, and perform bounded GitHub collaboration mutations when the current user request or an exact active standing authorization covers them and live permission allows them. One current instruction may authorize a bounded portfolio; derive an exact target and receipt for each item instead of asking for the same approval repeatedly. Before every external write, re-read the target, authorization, live permission, scope, repository rules, and idempotency or exact-head state. Never extend that authority to unrelated work, publication, release, access, secrets, rulesets, security disclosure, or provenance exceptions.
