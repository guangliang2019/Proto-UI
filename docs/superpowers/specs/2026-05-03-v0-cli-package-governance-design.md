...
- **cli-smoke**:`npm pack` CLI tarball,在 `mktemp -d` 干净目录里 `npm install` 真实 tarball + `react react-dom vue`,然后跑 `proto-ui --help / init --yes` 后顺序 `add react shadcn-button / add react base-button / add vue shadcn-button / add wc shadcn-button`(每次都真实 npm install,不带 `--no-install`),断言生成的 `proto-ui/config.json`、`components/{react,vue,wc}/index.ts`、根 `components/index.ts` 内容对齐决策 §4.2 / §4.5 / §4.6,并校验 `node_modules/@proto.ui/{adapter-react,adapter-vue,adapter-web-component,prototypes-shadcn,prototypes-base}` 真的落盘。最后 `npm install --no-save tsx @happy-dom/global-registrator`,把 `packages/cli/test/smoke-render/{react,vue,wc}.mjs` 拷进 SMOKE_DIR 用 `node --import tsx` 跑——三个文件都从 per-host facade(`./proto-ui/components/{react,vue,wc}/index.ts`) import,**避开**根 facade(根 facade 同时 re-export wc 模块,会在 module load 时 `extends HTMLElement` 把 React/Vue 子进程也带崩):
  - **react.mjs**:`@happy-dom/global-registrator` 注册 DOM globals → `react-dom/client` 把 `Button`(shadcn)和 `BaseButton`(base)分别 mount 到容器 → 50ms tick 等 prototype 生命周期挂载 → `container.querySelector('button')` 必须返回真实节点。**不用** `react-dom/server`,因为 v0 React adapter 是 client-only,SSR 只输出 `<div></div>` 占位符,测不到真实按钮。
  - **vue.mjs**:同样 happy-dom + `Vue.createApp({...}).mount(container)` + 50ms tick + `querySelector('button')`。Vue adapter 也是 client-only,SSR 只输出空 rootTag。
  ...
  这一段把 install + grep 之外的"adapter 行为插槽是否真插上、原型挂载是否真触发、customElements 注册是否真生效"也拦在 PR 上
...
| Clean-env smoke(`mktemp -d` 干净项目) | 🟡 待发包 | smoke 默认按"包已发"写,真实 npm install 4 次 add(react/vue/wc × shadcn/base 全覆盖),最后通过 `tsx` + `@happy-dom/global-registrator` + `react-dom/client` / `Vue.createApp().mount()` / wc 侧效 import 跑真实渲染器烟雾(React/Vue 都走 happy-dom 客户端挂载——v0 adapter 是 client-only,SSR 只输出空 rootTag,所以 server-renderer 路径会漏掉 prototype 真实输出)。本地用 `--no-install` 变体过过,facade 内容对齐决策 §4.2 / §4.5 / §4.6,5 个 `@proto.ui/*` 包真实落盘断言已加;CI 第一次跑要等维护者把 `@proto.ui/{cli@next,adapter-react,adapter-vue,adapter-web-component,prototypes-shadcn,prototypes-base}` 发到 npm 才能绿 |
