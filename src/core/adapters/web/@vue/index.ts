...
import { VueRuntime } from './interface';
...
export function createVueAdapter<Props extends {}, Exposes extends {} = {}>(runtime: VueRuntime, prototype: Prototype<Props, Exposes>) {
  const { Vue } = runtime;
 const VueAdapter = <Props extends {}, Exposes extends {} = {}>(
    ...
    return Vue.defineComponent({
      ...
      setup(props, { slots }) {
        ...
        const _p: PrototypeAPI<Props, Exposes> = {
          ...
          context: {
            ...
            get: <T>(context: Context<T>) => {
              ...
              const contextContainer = Vue.inject<any>(context.id);
        ...
        const _rootRef = Vue.ref<HTMLElement | null>(null);
        ...
        _pendingContextOperations.forEach((operation) => {
          if (operation.type === 'provide') {
            ...
            // 通过 provide 传递容器（不是值！）
            Vue.provide(operation.context.id, contextContainer);
          } else if (operation.type === 'watch') {
            const contextContainer = Vue.inject<any>(operation.context.id);
            if (contextContainer) {
              // 设置初始值
